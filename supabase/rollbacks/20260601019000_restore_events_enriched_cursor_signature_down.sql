-- Rollback for 20260601019000_restore_events_enriched_cursor_signature.sql
-- Reverts:
--   - DROP of offset-based public.events_enriched(uuid,text,int,int,uuid,uuid[],timestamptz,timestamptz)
--   - CREATE OR REPLACE of cursor-based public.events_enriched (9 params, with is_outdoor/parent_tips/image_attributions)
--   - REVOKE/GRANT on cursor-based overload
-- Restores the 018000 end-state: offset-based overload with enum cast.
-- Prior body sourced from 20260601018000_fix_events_enriched_enum_cast.sql.
-- Data-loss caveat: none — function-only change.

BEGIN;

-- 1. Drop the cursor-based overload installed by 019000.
DROP FUNCTION IF EXISTS public.events_enriched(uuid, text, uuid, uuid[], timestamptz, timestamptz, timestamptz, uuid, integer);

-- 2. Restore the 018000 offset-based overload (prior body verbatim).
