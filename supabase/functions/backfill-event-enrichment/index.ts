...
async function enrichOne(
  ...
  // Tag-keyed stock image fallback with provider chain.
  // Order: Pexels (200/hr → unlimited) → Pixabay (6K/hr) → Unsplash (50/hr).
  // Only fires when (a) the row needs images, (b) the scraper path didn't
  // produce any, (c) tag-queue has classified the event, and (d) we have
  ...
  // Provider-specific attribution handling: only Unsplash requires RPC call
  // with full attribution + download tracking. Pexels/Pixabay use simpler
  // direct table insert.
  if (imageSource === "unsplash" && stockResult) {
    ...
    const { error: updateErr } = await supabase.rpc("update_event_enrichment", {
      p_event_id: row.event_id,
      p_latitude: latitude,
      p_longitude: longitude,
