...
function withFetch(fakeFetch: typeof fetch, fn: () => Promise<void>): Promise<void> {
...
Deno.test("kickProcessSourceQueue resolves when the invoked function returns 200", async () => {
  const calls: string[] = [];
...
Deno.test("kickProcessSourceQueue throws when the invoked function returns non-2xx", async () => {
  let threw = false;
  await withFetch(
    (_input) => Promise.resolve(new Response("downstream error", { status: 503 })),
    async () => {
      try {
        await kickProcessSourceQueue("http://supabase.local", "service-key");
      } catch {
...
Deno.test("non-EdgeRuntime: awaiting the .catch-wrapped kick settles it before returning", async () => {
  let settled = false;
...
Deno.test("non-EdgeRuntime: errors from kick are swallowed by .catch and do not propagate", async () => {
  ...
  assert(caughtByHandler, ".catch must have run; errors must not propagate past await kick");
});
