...
Deno.test("invokeFunction posts service-role json to named edge function", async () => {
  ...
  assertEquals(String(request.input), "https://project.supabase.co/functions/v1/target-fn")
  assertEquals(request.init?.method, "POST")
  assertEquals((request.init?.headers as Record<string, string>).Authorization, "Bearer service")
  assertEquals(
    (request.init?.headers as Record<string, string>)["Content-Type"],
    "application/json"
  )
  assertEquals((request.init?.headers as Record<string, string>)["X-Cron-Run-Key"], "run-1")
